...
import com.apparitionhq.instasnap.billing.BillingSecurity.VerifiedPurchase;
...
public class BillingHelper {
	...
	//	Log.i(TAG, "[BillingHelper] requestPurchase()");
		...
		try {
			...
			Log.i(TAG, "[BillingHelper-requestPurchase] request:" + requestIndentifier + "   Response" + responseCode.toString());

			startBuyPageActivity(pendingIntent, new Intent(), activityContext);
		} catch (RemoteException e) {
			Log.e(TAG, "[BillingHelper-requestPurchase] Failed, internet error maybe", e);
			Log.e(TAG, "[BillingHelper-requestPurchase] Billing supported: " + isBillingSupported());
		}
	}
	...
	public static void getPurchaseInformation(String[] notifyIds){
		...
		try {
			...
			Integer responseCodeIndex 	= (Integer) response.get("RESPONSE_CODE");
			C.ResponseCode responseCode = C.ResponseCode.valueOf(responseCodeIndex);
			Log.i(TAG, "[BillingHelper-getPurchaseInformation] RequestID:" + requestIndentifier + "    Response code:" + responseCode.toString());
			
		} catch (RemoteException e) {
			Log.e(TAG, "[BillingHelper-getPurchaseInformation] Failed, internet error maybe", e);
			Log.e(TAG, "[BillingHelper-getPurchaseInformation] Billing supported: " + isBillingSupported());
		}
	}
	...
	public static void confirmTransaction(String[] notifyIds) {
		...
		try {
			...
			Log.i(TAG, "[BillingHelper-confirmTransaction] request:" + requestIndentifier + "    response:" + responseCode.toString());

		} catch (RemoteException e) {
			Log.e(TAG, "[BillingHelper-confirmTransaction] Failed, internet error maybe", e);
			Log.e(TAG, "[BillingHelper-confirmTransaction] Billing supported: " + isBillingSupported());
		}
	}
	...
	public static void restoreTransactions(){
		restoreTransactionInformation(BillingSecurity.generateNonce());		
	}
	...
	public static void restoreTransactionInformation(Long nonce) {
		...
		try {
			...
			Log.i(TAG, "[BillingHelper-restore] request:" + requestIndentifier + "   responseCode:" + responseCode.toString());

		} catch (Exception e) {
			Log.e(TAG, "[BillingHelper-restore] Failed, internet error maybe", e);
			Log.e(TAG, "[BillingHelper-restore] Billing supported: " + isBillingSupported());
		}
	}
	
	private static boolean amIDead() {
		if (mService == null || mContext == null) {
			Log.e(TAG, "[BillingHelper-amIDead] BillingHelper not fully instantiated");
			return true;
		} else {
			return false;
		}
	...
	private static void startBuyPageActivity(PendingIntent pendingIntent, Intent intent, Context context){
		// add above 2.0 implementation with reflection, for now just using 1.6 implem	
		...
		try {
			...
			Log.e(TAG, "[BillingHelper] startBuyPageActivity CanceledException");
		}
	}
	...
	public static void verifyPurchase(String signedData, String signature) {
		...
		if (purchases != null && !purchases.isEmpty()){
			...
			if (latestPurchase!=null && latestPurchase.notificationId!=null){
		...
		if(mCompletedHandler != null){
			...
			Log.e(TAG, "[BillingHelper] verifyPurchase error. Handler not instantiated. Have you called setCompletedHandler()?");
		}
